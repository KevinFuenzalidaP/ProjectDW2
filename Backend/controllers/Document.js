...
let controller = {
    ...
    getArticle: (req, res) => {
        ...
        if(id || id != undefined ){
            //buscar el articulo:
            Document.findById(id,(err, articulo)=>{
    ...
    updateArticle: (req, res) => {
        ...
        try{
            ...
            if (validate_content && validate_title){
                Document.findByIdAndUpdate({_id : articleId}, params, {new:true}, (err, articleUpdated)=> {
                    if (err){
                        return res.status(500).send({
                            status : 'NOK',
    ...
    deleteArticle: (req, res) => {
        var articleId = req.params.id;
        try{
            Document.findOneAndDelete({_id : articleId}, (err, articleRemoved)=> {
                if(err){
                    return res.status(500).send({
                        status : 'NOK',
    ...
    upload: (req, res) => {
        ...
        if (!ext_allowed.includes(file_extension)){
            ...
            Document.findOneAndUpdate({_id : articleId}, {image : file_name}, {new:true}, (err, articleUpdated)=>{
                if (err){
                    return res.status(500).send({
                        status : 'NOK',
                        message : 'Error al Actualizar'
